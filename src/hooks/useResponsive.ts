...
const useResponsive = () => {
    const [isMobileWidth, setIsMobileWidth] = useState<boolean>(window.innerWidth <= MOBILE_WIDTH_THRESHOLD)
    const [isDesktopWidth, setIsDesktopWidth] = useState<boolean>(window.innerWidth > MOBILE_WIDTH_THRESHOLD)
